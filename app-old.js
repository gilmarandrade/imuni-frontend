...
/**
 * Lê a aba Respostas e retorna os atendimentos formatados como um objeto
 * além de calcular as escalas de cada atendimento
 */
...
/**
 * Agrupa os atendimentos por idoso
 */
app.get('/docs/:id/atendimentosporidoso/range/:range', async function(req, res) {
    const googleClient = await getGoogleClient();
    sheets.spreadsheets.values.get({
        auth: googleClient,
        spreadsheetId: req.params.id,
        ...
        const atendimentosComEscala = atendimentos.map(atendimento => {
            return {
                atendimento,
        ...
        return res.json(atendimentosComEscala);
    });
});
